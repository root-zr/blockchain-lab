...
public class InvokeQuery {
    ...
    private String doQuery(String orgName, String userName, String functionName)
            throws IOException, ContractException {
    ...
    public String query(String functionName) throws Exception {
        String res = null;

        try {
            res = doQuery(ORGNAME_ORG1, USERNAME_ORG1, functionName);
            //doQuery(ORGNAME_ORG1, USERNAME_ORG1, "Hi", "NAME");
        } catch (IOException | ContractException e) {
            e.printStackTrace();
        }
